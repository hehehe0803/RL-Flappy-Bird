"""Implementation of SARSA Lambda tabular method
Discretises the state-action space to either 1dp or
to the nearest 0.05"""
...
class Agent:
    ...
    def choose_action(self, state) -> int: # epsilon greedy policy
        ...
        # choose discretisation option:
        state = (round(state[0], 1), round(state[1], 1)) # round to 1 decimal place
        # state = (round(state[0] / 0.05) * 0.05, round(state[1] / 0.05) * 0.05) # round to the nearest 0.05
    ...
    def learn(self, history):
        ...
        for state, action, reward, next_state, next_action in history:
            ...
            # choose discretisation option:
            state = (round(state[0], 1), round(state[1], 1))  # round x and y to 1 decimal place
            next_state = (round(next_state[0], 1), round(next_state[1], 1)) # round to 1 decimal place
            # state = (round(state[0] / 0.05) * 0.05, round(state[1] / 0.05) * 0.05)  # round to the nearest 0.05
            # next_state = (round(next_state[0] / 0.05) * 0.05, round(next_state[1] / 0.05) * 0.05)  # round  to the nearest 0.05
